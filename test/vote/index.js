...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = bench.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      var secretKey = bench.ECPair.fromSeed("secret");
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");

      var secondSecretKey = bench.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = bench.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.bench);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");

      var tx = createVote(secretKey, publicKeys);
      (tx).should.be.ok;
      (tx).should.be.type("object");
      (tx).should.have.property("recipientId").and.be.type("string").and.be.equal("B8q7GikfCz2B9eQfVKeuLNRPmf59iAZRB2");
    });

    it("should be deserialised correctly", function () {
      var deserialisedTx = bench.crypto.fromBytes(bench.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(bench.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = bench.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = bench.crypto.verifySecondSignature(vt, bench.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
