...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // bench = 0x25, bench_devnet = 0x30
...
function verify(transaction, network) {
	network = network || networks.bench;
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.bench);
